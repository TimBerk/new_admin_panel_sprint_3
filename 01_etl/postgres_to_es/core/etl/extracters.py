...
from core.etl.transfers import PostgresTransform
...
class ElasticExtract(ABC):
    ...
    def load_data(self):
        limit = BLOCK_SIZE
        state_table = {}
        state_table_raw = self.state.get_state(self.table_name)
        ...
        for modified_ids in self.pg.chunk_read_table_id(self.table_name, date_start, limit, offset_start):
            ...
            film_result = self.pg.get_film_data([item['id'] for item in film_modified_ids])
            if film_result:
        ...
        self.state.set_state(self.table_name, json.dumps({
            'offset': 0,
            'date': date_end if isinstance(date_end, str) else date_end.strftime(FORMAT_DATE)
        }))
        return self.state
